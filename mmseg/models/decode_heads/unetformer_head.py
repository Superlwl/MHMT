...
class UNetFormerHead(BaseDecodeHead):
    def __init__(self,
        ...
        self.decoder = Decoder(in_channels, channels, dropout, window_size, num_classes)
...
if __name__ == '__main__':
    model = UNetFormerHead()
    # print(model)
    inputs = []

    input1 = torch.FloatTensor(np.random.rand(1, 64, 128, 128))
    input2 = torch.FloatTensor(np.random.rand(1, 128, 64, 64))
    input3 = torch.FloatTensor(np.random.rand(1, 256, 32, 32))
    input4 = torch.FloatTensor(np.random.rand(1, 512, 16, 16))
